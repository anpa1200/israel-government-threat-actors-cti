...
- Tool type: Dual-use tunneling / proxy tooling
- Confidence: Medium
...
MITRE reports Magic Hound use of Fast Reverse Proxy and Plink-style tooling for proxying, masqueraded access, and non-standard C2 routing.

## Hash And IOC Status

- Status: No malware hash; dual-use binary monitoring and local allowlisting required.
...
Hunt FRP/Plink execution from user paths or server paths, masqueraded filenames, non-standard ports, and unauthorized proxy sessions.

## Handling Notes

Dual-use; tool presence alone is not attribution.
